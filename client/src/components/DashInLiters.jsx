...
const WeeklyCard = styled(MetricCard)({
  '&::before': {
...
export default function DashInLiters() { 
  ...
  const metricOptions = [
    { value: "total_actuals", label: "Actuals", icon: <LocalGasStation />, color: "#f44336" },
    { value: "total_sales", label: "Sales", icon: <LocalGasStation />, color: "#9c27b0"},
    { value: "total_variation", label: "Variation", icon: <CompareArrows />, color: "#9c27b0"}
  ];
  ...
  const getKpiTargets = () => {
    ...
    return baseTargets[selectedMetric] || baseTargets.total_sales;
  };
  ...
  const calculateKPIs = (filteredData) => {
    ...
    */}
    ...
    // In DashInLiters, we need to check which filters are available
    // Does this component have day/month filters?
    // Check for the month filter variable
    if (typeof month !== 'undefined' && month && month.length > 0) {
      selectedMonth = parseInt(month);
      console.log('Using custom month filter:', selectedMonth);
    }
    
    // Check for the day filter variable
    if (typeof day !== 'undefined' && day && day.length > 0) {
      selectedDay = parseInt(day);
      console.log('Using custom day filter:', selectedDay);
    }
    
    // If this component doesn't have day/month filters in the UI,
    // you might need to extract them from the data or api parameters
  ...
  const getColumnOptions = () => {
    ...
    return Object.keys(sample).filter(key => key == 'branch' || key == 'sales_ago' || key == 'sales_pms' || key =='total_sales' || key == 'actuals_ago' 
        || key == 'actuals_pms' || key == 'total_actuals' || key == 'variation_ago' || key == 'variation_pms' || key == 'total_variation' 
     );
  };
            ...
            <Grid container spacing={6}>
                ...
                <DailyCard>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'baseline', mb: 1 }}>
                    <Typography variant="h4" fontWeight="bold" sx={{ mr: 1 }}>
                      {kpis.dailyValue.toLocaleString('en-US')}
                    </Typography>
                  ...
                  <Box sx={{ mt: 1.5 }}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
                      <Typography variant="body2" color="text.secondary">
                        Target: {getKpiTargets().daily.toLocaleString('en-US')}
                      </Typography>
              ...
              <Grid item size={{ xs: 6, md: 4 }}>
                <MonthlyCard>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'baseline', mb: 1 }}>
                    <Typography variant="h4" fontWeight="bold" sx={{ mr: 1 }}>
                      {kpis.monthlyValue.toLocaleString('en-US')}
                    </Typography>
                  </Box>
                  ...
                  <Box sx={{ mt: 1.5 }}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
                      <Typography variant="body2" color="text.secondary">
                        Target: {getKpiTargets().monthly.toLocaleString('en-US')}
                      </Typography>
                      <Typography variant="body2" color="primary" fontWeight="medium">
